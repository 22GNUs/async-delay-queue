...
public interface ReactiveDelayQueue<T> {
   ...
   * @return Flux<T> or Flux.empty()
   */
  Flux<T> dequeueBatch(long max, long limit);
   ...
   * @return Flux<T> or Flux.empty()
   */
  Flux<T> dequeueBatch(long min, long max, long offset, long limit);
